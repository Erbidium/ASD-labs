...
void fillAdjacencyMatrix(vector<vector<int>>& adjacencyMatrix);				//заповнення матриці та інтерфейс для користувача
void manualInputdAjacencyMatrix(vector<vector<int>>& adjacencyMatrix);		//ручне заповнення матриці
void randomFillAjacencyMatrix(vector<vector<int>>& adjacencyMatrix);		//генерація матриці випадковими числами
void printMatrix(const vector<vector<int>>& matrix, bool isMatrixP=false);  //виведення матриці на екран
int getSum(int number1, int number2);										//сума двох елементів
void DantzigAlgorithm(vector<vector<int>>& adjacencyMatrix);				//алгоритм Данцига(зовнішній спосіб визначення шляхів)
...
void DantzigAlgorithm(vector<vector<int>>& adjacencyMatrix)
{
	...
	for(int m=1;m<=numberVerticles;m++)
	{
		...
		for(int i=0;i<m-1;i++)
		{
			...
			for(int j=0;j<m-1;j++)
			{
				matrixD[m][i][j]=min(getSum(matrixD[m][i][m-1], matrixD[m][m-1][j]), matrixD[m-1][i][j]);
	...
	for(int i=0;i<numberVerticles;i++)
	{
		for(int j=0;j<numberVerticles;j++)
		{
			if((matrixD[numberVerticles][i][j]!=inf)&&(matrixD[numberVerticles][i][j]!=0))
			{
				...
				{
					if((matrixD[numberVerticles][i][k]+matrixD[0][k][j]==matrixD[numberVerticles][i][j])&&(flag==false)&&(k!=j))
					{
						matrixP[i][j]=k+1;
						flag=true;
					}
